...
pub(crate) fn read_auth_header() -> Result<&'static str> {
    if let Ok(mut key) = std::env::var(OPENCODE_API_KEY_ENV) {
        // SAFETY: process env is read at startup before request threads spawn.
        unsafe { std::env::remove_var(OPENCODE_API_KEY_ENV) };
        let trimmed = key.trim().to_string();
        // Zeroize the raw env String immediately: it is an unprotected copy
...
mod tests {
    ...
    fn env_read_immediately_unsets_the_variable() {
        ...
        unsafe { std::env::set_var(OPENCODE_API_KEY_ENV, "  valid-key-42  ") };
        read_auth_header().expect("env key must read");
        assert!(
            std::env::var(OPENCODE_API_KEY_ENV).is_err(),
    ...
    fn env_read_zeroizes_the_raw_heap_string() {
        // Zeroizing an unprotected key copy scrubs its bytes: this is what
        // read_auth_header does to the String std::env::var allocated.
