//! Translate Chat Chat Completions streaming SSE into OAI Responses
//! streaming SSE.
//!
...
struct ToolCallState {
    /// Stable unique ID used in OAI events (Chat's tool call id).
    oai_id: String,
...
/// A `Read` implementation that translates Chat Chat SSE into OAI Responses
/// SSE on the fly.
pub(crate) struct ChatToOaiStream {
    model: String,
    resp_id: String,
    created_at: u64,
    seq: u64,
    ...
    /// `incomplete_details.reason: "max_output_tokens"` when Chat truncates
    /// at `max_tokens`.
...
impl ChatToOaiStream {
    ...
    fn process_line(&mut self, line: &str) {
        ...
        let ev: Value = match serde_json::from_str(payload) {
            Ok(v) => v,
            Err(e) => {
                eprintln!("{}: non-JSON SSE: {e} — {payload}", self.log_prefix);
                return;
            }
        };
        ...
        // Open the response on the first processed choice chunk. Chat does
    ...
    fn emit_completed(&mut self) {
        ...
        // Determine the terminal status from the finish reason. Chat's
...
mod tests {
    ...
    fn stream(model: &str) -> ChatToOaiStream {
        ChatToOaiStream::new(
            model.to_string(),
            Box::new(&b""[..]),
            /*verbose_req*/ None,
    ...
    fn drain_events(t: &mut ChatToOaiStream) -> Vec<(String, Value)> {
