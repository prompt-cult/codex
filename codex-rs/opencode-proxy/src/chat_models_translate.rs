...
/// Fallback context window used when Chat omits `max_context_length`.
...
/// Top-level Chat `/models` envelope. Extra fields are ignored.
#[derive(Debug, Deserialize)]
struct ChatModelList {
    #[serde(default)]
    data: Vec<ChatModel>,
}
...
/// Result of translating a Chat `/models` payload.
#[derive(Debug)]
...
/// Parse raw Chat `/models` JSON and build a codex [`ModelsResponse`].
///
...
pub fn translate_chat_models(
    raw: &[u8],
    exclude: &globset::GlobSet,
    overrides: &std::collections::HashMap<String, ModelTranslateOverride>,
) -> Result<TranslatedModels> {
    let list: ChatModelList =
        serde_json::from_slice(raw).context("parsing Chat /models response")?;
...
/// Build a fully-populated [`ModelInfo`] for a chat-capable Chat model.
///
/// `ModelInfo` has no `Default`, so every field is set explicitly. Optional and
/// reasoning-related metadata Chat does not provide is left empty/`None`;
...
fn model_info_for(index: usize, m: ChatModel, ovr: ModelTranslateOverride) -> ModelInfo {
    ModelInfo {
        slug: m.id.clone(),
        display_name: m.id,
        description: None,
...
mod tests {
    ...
    const FIXTURE: &str = include_str!("../tests/fixtures/chat_models.json");
    ...
    fn filters_to_chat_capable_models() {
        let out = translate_chat_models(FIXTURE.as_bytes(), &no_exclusions(), &no_overrides())
            .expect("translate");
        // Fixture: zai-glm-5-2 and chat-medium-latest are chat; chat-embed is not.
        assert_eq!(out.response.models.len(), 2);
        assert_eq!(out.chat_loaded, 2);
        assert!(out.response.models.iter().all(|m| m.slug != "chat-embed"));
    }

    #[test]
    fn exclusion_globs_drop_matching_models() {
        let exclude = globset_of(&["zai-glm-5-2", "*-medium-*"]);
        let out = translate_chat_models(FIXTURE.as_bytes(), &exclude, &no_overrides())
            .expect("translate");
    ...
    fn exact_exclusion_keeps_prefixed_variant() {
        ...
        let out = translate_chat_models(raw, &exclude, &no_overrides()).expect("translate");
    ...
    fn defaults_context_window_when_missing() {
        ...
        let medium = out
            ...
            .models
            .iter()
            .find(|m| m.slug == "chat-medium-latest")
    ...
    fn override_replaces_base_instructions_for_one_model() {
        ...
        let out = translate_chat_models(FIXTURE.as_bytes(), &no_exclusions(), &overrides)
            .expect("translate");
        let glm = out
            .response
            .models
